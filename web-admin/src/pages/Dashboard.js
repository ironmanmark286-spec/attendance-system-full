...
export default function Dashboard({ theme, onToggleTheme }) {
  const [activeNav, setActiveNav] = useState("overview"); // 'overview', 'attendance', 'leaves', 'helpdesk', 'employees', 'ai_insights'
  ...
  const [copilotMessages, setCopilotMessages] = useState([
    { role: 'ai', text: "Namaste! Main aapka Smart AI Assistant hoon. Bina kisi setup ke aap mujhe commands de sakte hain jaise: 'Approve leaves', 'Download report', 'Dark theme' ya 'Add new employee'." }
  ]);
  ...
  const handleCopilotSubmit = async (e) => {
    ...
    setCopilotMessages(prev => [...prev, { role: 'ai', text: "Thinking..." }]);

    // Local Smart Assistant Logic (No API needed)
    setTimeout(async () => {
      let aiResponse = "Main samajh nahi paaya. Aap mujhe 'approve leaves', 'show late employees', 'export attendance', ya 'add new employee' jaisi commands de sakte hain.";
      ...
      if (lower.includes("approve") || lower.includes("chutti") || lower.includes("leave")) {
        ...
        if (pending.length > 0) {
          ...
          await loadData();
          aiResponse = `Hogaya! Maine ${pending.length} pending leaves approve kar di hain.`;
          setActiveNav('leaves');
        } else {
          aiResponse = "Abhi system mein koi pending leave request nahi hai.";
        }
      } else if (lower.includes("late") || lower.includes("der")) {
        setActiveNav('attendance');
        setSearchQuery("LATE");
        aiResponse = "Yeh lijiye, maine un employees ki list filter kar di hai jo aaj late hain.";
      } else if (lower.includes("export") || lower.includes("csv") || lower.includes("download") || lower.includes("report")) {
        downloadCsv();
        aiResponse = "Report generate ho rahi hai. Excel/CSV file ka download shuru ho gaya hai.";
      } else if (lower.includes("add") || lower.includes("new employee") || lower.includes("naya") || lower.includes("onboard")) {
        setActiveNav('employees');
        setIsAddEmpModalOpen(true);
        aiResponse = "Naye employee ko add karne ka form open kar diya hai. Kripya details bharein.";
      } else if (lower.includes("ticket") || lower.includes("helpdesk") || lower.includes("support") || lower.includes("issue")) {
        setActiveNav('helpdesk');
        aiResponse = "Main aapko IT & HR Helpdesk section me le aaya hoon.";
      } else if (lower.includes("dark") || lower.includes("light") || lower.includes("theme")) {
        onToggleTheme();
        aiResponse = "Maine website ka theme change kar diya hai!";
      } else if (lower.includes("employee") || lower.includes("team") || lower.includes("staff")) {
        setActiveNav('employees');
        aiResponse = "Employee directory open kar di gayi hai.";
      } else if (lower.includes("hello") || lower.includes("hi") || lower.includes("namaste") || lower.includes("hey")) {
        aiResponse = "Namaste Admin! Main aapka local AI assistant hoon. Bina kisi API ke, main aapke HR tasks automate kar sakta hoon. Bataiye kya karna hai?";
      }

      setCopilotMessages(prev => {
        const newArr = [...prev];
        newArr[newArr.length - 1] = { role: 'ai', text: aiResponse };
        return newArr;
      });
    }, 600); // 600ms delay to make it feel like AI is thinking
  };
  ...
  const bentoCardStyle = { background: 'var(--bg-card)', borderRadius: 32, padding: 32, border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)', display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden', transition: 'all 0.3s' };
  ...
  return (
    <div className="app-layout">
      ...
      <aside className="sidebar" style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
        <div className="brand">
          <div style={{ width: 36, height: 36, background: 'linear-gradient(135deg, var(--primary), #a855f7)', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', boxShadow: '0 8px 16px rgba(99, 102, 241, 0.4)' }}>
            <Activity size={20} />
          </div>
          PulseHR
        </div>
        ...
        <nav className="nav-menu" style={{ flex: 1, overflowY: 'auto' }}>
          ...
          <button className={`nav-item ${activeNav === 'ai_insights' ? 'active' : ''}`} onClick={() => setActiveNav('ai_insights')}>
            <BrainCircuit size={20} /> AI Insights <span style={{fontSize: 10, background: '#a855f7', color: '#fff', padding: '2px 6px', borderRadius: 10, marginLeft: 'auto'}}>NEW</span>
          </button>
      ...
      <main className="main-content">
        ...
        <header className="topbar">
          ...
          <div className="user-profile">
            <button className="btn btn-secondary" style={{ padding: 10, borderRadius: 12 }} onClick={loadData}>
              <RefreshCw size={18} className={loading ? "spin" : ""} />
            </button>
            <div style={{ position: 'relative' }}>
              <div style={{ width: 44, height: 44, borderRadius: 14, background: 'var(--bg-input)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-main)', cursor: 'pointer' }} onClick={() => setShowNotifications(!showNotifications)}>
                <Bell size={20} />
              </div>
              {showNotifications && (
                <div style={{ position: 'absolute', top: 54, right: 0, width: 300, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, boxShadow: '0 10px 30px rgba(0,0,0,0.1)', zIndex: 100, padding: 16 }}>
                  <h4 style={{ margin: '0 0 12px 0', fontSize: 16, fontWeight: 700 }}>Notifications</h4>
                  <div style={{ padding: 12, background: 'var(--bg-input)', borderRadius: 12, fontSize: 14, color: 'var(--text-muted)' }}>
                    No new notifications right now.
                  </div>
                </div>
              )}
            </div>
            <div className="avatar">{stats.adminName.charAt(0).toUpperCase()}</div>
            <div>
          ...
          {activeNav === "overview" && (
            <>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: 24, marginBottom: 40 }}>
                ...
                <div style={{ ...bentoCardStyle, gridColumn: 'span 8' }}>
                  <div style={{ position: 'absolute', top: '-50%', right: '-10%', width: '400px', height: '400px', background: 'var(--primary)', filter: 'blur(120px)', opacity: 0.15, borderRadius: '50%', zIndex: 0 }}></div>
                  <div style={{ position: 'absolute', bottom: '-20%', right: '20%', width: '300px', height: '300px', background: '#a855f7', filter: 'blur(100px)', opacity: 0.15, borderRadius: '50%', zIndex: 0 }}></div>
                  
                  <div style={{ position: 'relative', zIndex: 1 }}>
                    <h1 style={{ fontSize: 38, fontWeight: 900, color: 'var(--text-main)', letterSpacing: -1, marginBottom: 8, textTransform: 'capitalize' }}>{greeting}, {stats.adminName}.</h1>
                    <p style={{ color: 'var(--text-muted)', fontSize: 16, fontWeight: 500, marginBottom: 36 }}>Your workforce at <strong style={{ color: 'var(--text-main)' }}>{stats.companyName} {stats.companyCode ? `(${stats.companyCode})` : ""}</strong> is operating at peak efficiency today.</p>
                  </div>
                  
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginTop: 'auto', position: 'relative', zIndex: 1 }}>
                    <div style={{ background: 'var(--bg-app)', border: '1px solid var(--border)', padding: 20, borderRadius: 24 }}>
                    ...
                    <div style={{ background: 'var(--bg-app)', border: '1px solid var(--border)', borderTop: '4px solid var(--success)', padding: 20, borderRadius: 24 }}>
                    ...
                    <div style={{ background: 'var(--bg-app)', border: '1px solid var(--border)', borderTop: '4px solid var(--warning)', padding: 20, borderRadius: 24 }}>
                    ...
                    <div style={{ background: 'var(--bg-app)', border: '1px solid var(--border)', borderTop: '4px solid var(--danger)', padding: 20, borderRadius: 24 }}>
                ...
                <div style={{ ...bentoCardStyle, gridColumn: 'span 4', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
                  <div style={{ position: 'absolute', top: 20, left: 20 }}><Award size={24} color="var(--warning)" /></div>
                  <div style={{ width: 80, height: 80, borderRadius: 40, background: 'linear-gradient(135deg, #f59e0b, #ef4444)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 32, fontWeight: 900, marginBottom: 16, boxShadow: '0 10px 20px rgba(245, 158, 11, 0.3)' }}>
                    {topPerformer.avatar}
                  </div>
                  ...
                  <div style={{ marginTop: 24, background: 'var(--bg-input)', padding: '10px 20px', borderRadius: 20, color: 'var(--success)', fontWeight: 800, fontSize: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <div style={{ ...bentoCardStyle, gridColumn: 'span 4' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
                    ...
                    <span style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 800, color: 'var(--success)', background: 'rgba(16, 185, 129, 0.1)', padding: '4px 10px', borderRadius: 20 }}><div style={{ width: 6, height: 6, borderRadius: 3, background: 'var(--success)' }}></div> ONLINE</span>
                  </div>
                ...
                <div style={{ ...bentoCardStyle, gridColumn: 'span 4', background: 'var(--bg-hover)' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                    <button style={{ padding: 16, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, cursor: 'pointer', transition: 'transform 0.2s' }} onClick={() => {setActiveNav('employees'); setIsAddEmpModalOpen(true);}} onMouseOver={(e) => e.currentTarget.style.transform = 'scale(1.05)'} onMouseOut={(e) => e.currentTarget.style.transform = 'scale(1)'}>
                      <UserPlus size={24} color="var(--primary)" />
                      <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-main)' }}>Onboard</span>
                    </button>
                    <button style={{ padding: 16, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, cursor: 'pointer', transition: 'transform 0.2s' }} onClick={() => setActiveNav('helpdesk')} onMouseOver={(e) => e.currentTarget.style.transform = 'scale(1.05)'} onMouseOut={(e) => e.currentTarget.style.transform = 'scale(1)'}>
                      <Headset size={24} color="var(--success)" />
                      <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-main)' }}>Support</span>
                    </button>
                    <button style={{ padding: 16, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, cursor: 'pointer', transition: 'transform 0.2s', gridColumn: 'span 2' }} onClick={downloadCsv} onMouseOver={(e) => e.currentTarget.style.transform = 'scale(1.02)'} onMouseOut={(e) => e.currentTarget.style.transform = 'scale(1)'}>
                ...
                <div style={{ ...bentoCardStyle, gridColumn: 'span 4', background: 'linear-gradient(135deg, var(--bg-card), var(--bg-hover))' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                    {worldClocks.map((clock, idx) => (
                      <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 16, background: 'var(--bg-input)', borderRadius: 16 }}>
                ...
                <div style={{ ...bentoCardStyle, gridColumn: 'span 8', padding: 0 }}>
                  <div className="table-toolbar">
                    <h3 style={{ fontSize: 18, fontWeight: 700 }}>Live Check-ins</h3>
                    <button className="btn btn-secondary" onClick={() => setActiveNav('attendance')}>View Detailed Logs</button>
                  </div>
                ...
                <div style={{ ...bentoCardStyle, gridColumn: 'span 4' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 16, maxHeight: 250, overflowY: 'auto', paddingRight: 8 }}>
                    {notices.length > 0 ? notices.map(notice => (
                      <div key={notice.id} style={{ padding: 16, background: 'var(--bg-input)', borderRadius: 12, borderLeft: `4px solid ${notice.type === 'Upcoming Holiday' ? 'var(--warning)' : 'var(--primary)'}`, position: 'relative' }}>
                        <button 
          ...
          {activeNav === "ai_insights" && (
            <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 24, marginBottom: 24 }}>
                {/* Priority Alert Box */}
                <div className="card" style={{ gridColumn: 'span 2', background: 'linear-gradient(135deg, rgba(239, 68, 68, 0.1), rgba(245, 158, 11, 0.1))', borderColor: 'rgba(239, 68, 68, 0.3)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <div style={{ background: 'var(--danger)', padding: 12, borderRadius: 12, color: '#fff', boxShadow: '0 8px 16px rgba(239, 68, 68, 0.3)' }}><ShieldAlert size={24}/></div>
                      <h3 style={{ fontSize: 22, fontWeight: 800 }}>Critical Anomalies Detected</h3>
                    </div>
                    <span style={{ background: 'var(--danger)', color: '#fff', padding: '4px 12px', borderRadius: 20, fontSize: 12, fontWeight: 800 }}>2 ALERTS</span>
                  </div>
                ...
                {/* AI Action Plan */}
                <div className="card" style={{ background: 'var(--bg-card)', border: '1px solid var(--primary)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
                    <Lightbulb size={24} color="var(--warning)" />
                    <h3 style={{ fontSize: 18, fontWeight: 800 }}>AI Action Plan</h3>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    <label style={{ display: 'flex', gap: 10, alignItems: 'flex-start', padding: 12, background: 'var(--bg-input)', borderRadius: 12, cursor: 'pointer' }}>
                  ...
                  <div style={{ height: 280, width: '100%' }}>
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={anomalyData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                        ...
                        <RechartsTooltip contentStyle={{ background: 'var(--bg-card)', borderColor: 'var(--border)', borderRadius: 12, boxShadow: '0 10px 20px rgba(0,0,0,0.1)' }} />
                  ...
                  <div style={{ height: 280, width: '100%' }}>
                    <ResponsiveContainer width="100%" height="100%">
                      <RadarChart cx="50%" cy="50%" outerRadius="70%" data={deptHealthData}>
                        ...
                        <RechartsTooltip contentStyle={{ background: 'var(--bg-card)', borderColor: 'var(--border)', borderRadius: 12 }} />
                      </RadarChart>
                    </ResponsiveContainer>
                  </div>
                </div>
            ...
            <>
              <div className="page-header">
                ...
                <div style={{ display: 'flex', gap: 12 }}>
                  <button className="btn btn-secondary" onClick={handleGenerateFakeData}>
                  ...
                  <button className="btn" style={{ background: '#0f172a' }} onClick={downloadCsv}>
                          ...
                          <td>
                            {l.status === 'PENDING' ? (
                              <div style={{ display: 'flex', gap: 10 }}>
                                <button className="btn" style={{ padding: '8px 12px', background: 'var(--success)', boxShadow: '0 4px 12px rgba(16,185,129,0.3)' }} onClick={() => handleLeaveAction(l.id, 'APPROVED')}>
                                  <Check size={16} /> Approve
                                </button>
                                <button className="btn" style={{ padding: '8px 12px', background: 'var(--danger)', boxShadow: '0 4px 12px rgba(239,68,68,0.3)' }} onClick={() => handleLeaveAction(l.id, 'REJECTED')}>
                                  <X size={16} /> Reject
                                </button>
                              </div>
                            ) : (
                    ...
                    {filteredTickets.length === 0 ? (
                      ...
                      filteredTickets.map((t) => (
                        <tr key={t.id}>
                          ...
                          <td><div style={{ fontWeight: 700 }}>{t.employee}</div></td>
                          <td style={{ maxWidth: 250, whiteSpace: 'normal', lineHeight: 1.5 }}>{t.issue}</td>
                          <td><span style={{ fontWeight: 700, color: t.priority === 'HIGH' ? 'var(--danger)' : t.priority === 'MEDIUM' ? 'var(--warning)' : 'var(--success)' }}>{t.priority}</span></td>
                          <td><span style={{ fontWeight: 600, fontSize: 12, padding: '4px 10px', borderRadius: 20, border: '1px solid var(--border)' }}>{t.status}</span></td>
                          <td>
                            <button className="btn btn-secondary" style={{ padding: '8px 12px' }}>Reply & Resolve</button>
                          </td>
                        </tr>
                      ))
                    )}
            ...
            <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 24 }}>
                {filteredEmployees.length === 0 ? (
                  <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '60px', color: 'var(--text-muted)', background: 'var(--bg-card)', borderRadius: 24, border: '1px dashed var(--border)' }}>
                      ...
                      <div key={e.id} className="employee-card" onClick={() => setSelectedEmpInfo(e)}>
                        ...
                        <div className="employee-card-body">
                          ...
                          <div className="employee-card-profile">
                            <div className="employee-avatar" style={{ background: isActive ? 'linear-gradient(135deg, var(--primary), #a855f7)' : 'linear-gradient(135deg, var(--text-muted), #475569)' }}>
            ...
            <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: 24 }}>
                ...
                <div className="card">
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 16, background: 'var(--bg-input)', borderRadius: 16, border: '1px solid var(--border)' }}>
                    <div>
                    ...
                    <label style={{ position: 'relative', display: 'inline-block', width: 52, height: 30 }}>
                      ...
                      <span style={{ position: 'absolute', cursor: 'pointer', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: theme === 'dark' ? 'var(--primary)' : 'var(--border)', borderRadius: 34, transition: '0.3s' }}>
                        <span style={{ position: 'absolute', height: 22, width: 22, left: 4, bottom: 4, backgroundColor: '#fff', borderRadius: '50%', transition: '0.3s', transform: theme === 'dark' ? 'translateX(22px)' : 'none', boxShadow: '0 2px 5px rgba(0,0,0,0.2)' }}></span>
                      </span>
                    </label>
                  </div>

                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 16, background: 'var(--bg-input)', borderRadius: 16, border: '1px solid var(--border)', marginTop: 12 }}>
                    <div>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      {['#6366f1', '#10b981', '#f59e0b', '#ef4444', '#ec4899', '#3b82f6'].map(color => (
                        <button key={color} onClick={() => setAccentColor(color)} title="Change Theme Color" style={{ width: 28, height: 28, borderRadius: 14, backgroundColor: color, border: accentColor === color ? '3px solid var(--bg-card)' : 'none', outline: accentColor === color ? `2px solid ${color}` : 'none', cursor: 'pointer', transition: 'transform 0.2s' }} onMouseOver={e => e.currentTarget.style.transform = 'scale(1.1)'} onMouseOut={e => e.currentTarget.style.transform = 'scale(1)'} />
                      ))}
                    </div>
                  </div>
                </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 16, background: 'var(--bg-input)', borderRadius: 16, border: '1px solid var(--border)' }}>
                      <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
                        <div style={{ background: 'var(--bg-card)', padding: 10, borderRadius: 12, border: '1px solid var(--border)' }}><Globe2 size={20} color="var(--success)" /></div>
                        <div>
                      ...
                      <select style={{ padding: '8px 12px', borderRadius: 12, background: 'var(--bg-card)', color: 'var(--text-main)', border: '1px solid var(--border)', outline: 'none', fontWeight: 600, cursor: 'pointer' }} value={language} onChange={(e) => setLanguage(e.target.value)}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 16, background: 'var(--bg-input)', borderRadius: 16, border: '1px solid var(--border)' }}>
                      <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
                        <div style={{ background: 'var(--bg-card)', padding: 10, borderRadius: 12, border: '1px solid var(--border)' }}><Monitor size={20} color="var(--warning)" /></div>
                        <div>
                      ...
                      <label style={{ position: 'relative', display: 'inline-block', width: 52, height: 30 }}>
                        ...
                        <span style={{ position: 'absolute', cursor: 'pointer', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: compactMode ? 'var(--primary)' : 'var(--border)', borderRadius: 34, transition: '0.3s' }}>
                          <span style={{ position: 'absolute', height: 22, width: 22, left: 4, bottom: 4, backgroundColor: '#fff', borderRadius: '50%', transition: '0.3s', transform: compactMode ? 'translateX(22px)' : 'none', boxShadow: '0 2px 5px rgba(0,0,0,0.2)' }}></span>
                        </span>
                      </label>
                    </div>
                  </div>
                ...
                <div className="card">
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 16, background: 'var(--bg-input)', borderRadius: 16, border: '1px solid var(--border)' }}>
                      <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
                        <div style={{ background: 'var(--bg-card)', padding: 10, borderRadius: 12, border: '1px solid var(--border)' }}><Mail size={20} color="var(--primary)" /></div>
                        <div>
                      ...
                      <label style={{ position: 'relative', display: 'inline-block', width: 52, height: 30 }}>
                        ...
                        <span style={{ position: 'absolute', cursor: 'pointer', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: emailNotif ? 'var(--success)' : 'var(--border)', borderRadius: 34, transition: '0.3s' }}>
                          <span style={{ position: 'absolute', height: 22, width: 22, left: 4, bottom: 4, backgroundColor: '#fff', borderRadius: '50%', transition: '0.3s', transform: emailNotif ? 'translateX(22px)' : 'none', boxShadow: '0 2px 5px rgba(0,0,0,0.2)' }}></span>
                        </span>
                      </label>
                    </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 16, background: 'var(--bg-input)', borderRadius: 16, border: '1px solid var(--border)' }}>
                      <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
                        <div style={{ background: 'var(--bg-card)', padding: 10, borderRadius: 12, border: '1px solid var(--border)' }}><Smartphone size={20} color="var(--warning)" /></div>
                        <div>
                      ...
                      <label style={{ position: 'relative', display: 'inline-block', width: 52, height: 30 }}>
                        ...
                        <span style={{ position: 'absolute', cursor: 'pointer', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: pushNotif ? 'var(--success)' : 'var(--border)', borderRadius: 34, transition: '0.3s' }}>
                          <span style={{ position: 'absolute', height: 22, width: 22, left: 4, bottom: 4, backgroundColor: '#fff', borderRadius: '50%', transition: '0.3s', transform: pushNotif ? 'translateX(22px)' : 'none', boxShadow: '0 2px 5px rgba(0,0,0,0.2)' }}></span>
                        </span>
                      </label>
                    </div>
                  </div>
                ...
                <div className="card">
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 16, background: 'var(--bg-input)', borderRadius: 16, border: '1px solid var(--border)' }}>
                      <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
                        <div style={{ background: 'var(--bg-card)', padding: 10, borderRadius: 12, border: '1px solid var(--border)' }}><Activity size={20} color="var(--success)" /></div>
                        <div>
                      ...
                      <label style={{ position: 'relative', display: 'inline-block', width: 52, height: 30 }}>
                        ...
                        <span style={{ position: 'absolute', cursor: 'pointer', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: enableOT ? 'var(--success)' : 'var(--border)', borderRadius: 34, transition: '0.3s' }}>
                          <span style={{ position: 'absolute', height: 22, width: 22, left: 4, bottom: 4, backgroundColor: '#fff', borderRadius: '50%', transition: '0.3s', transform: enableOT ? 'translateX(22px)' : 'none', boxShadow: '0 2px 5px rgba(0,0,0,0.2)' }}></span>
                        </span>
                      </label>
                    </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 16, background: 'var(--bg-input)', borderRadius: 16, border: '1px solid var(--border)' }}>
                      <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
                        <div style={{ background: 'var(--bg-card)', padding: 10, borderRadius: 12, border: '1px solid var(--border)' }}><Clock size={20} color="var(--warning)" /></div>
                        <div>
                      ...
                      <select style={{ padding: '8px 12px', borderRadius: 12, background: 'var(--bg-card)', color: 'var(--text-main)', border: '1px solid var(--border)', outline: 'none', fontWeight: 600, cursor: 'pointer' }} value={standardWorkHours} onChange={(e) => setStandardWorkHours(Number(e.target.value))}>
        ...
        <div className="modal-overlay" onClick={(e) => { if(e.target.className === 'modal-overlay') { setIsAddEmpModalOpen(false); setAddedEmpDetails(null); } }}>
          <div className="modal-content">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32 }}>
              ...
              <button className="btn btn-secondary" style={{ padding: 8, borderRadius: 12, border: 'none' }} onClick={() => { setIsAddEmpModalOpen(false); setAddedEmpDetails(null); }}>
                <X size={24} />
              </button>
            </div>
            ...
            {addedEmpDetails ? (
              <div style={{ textAlign: 'center' }}>
                <div style={{ width: 64, height: 64, background: 'var(--success)', color: '#fff', borderRadius: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px' }}>
                  <Check size={32} />
                </div>
                ...
                <div style={{ background: 'var(--bg-input)', padding: 20, borderRadius: 16, textAlign: 'left', display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div className="modal-overlay" onClick={(e) => { if(e.target.className === 'modal-overlay') setIsEditCompanyModalOpen(false) }}>
          <div className="modal-content" style={{ maxWidth: 400 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32 }}>
              ...
              <button className="btn btn-secondary" style={{ padding: 8, borderRadius: 12, border: 'none' }} onClick={() => setIsEditCompanyModalOpen(false)}>
                <X size={24} />
              </button>
            </div>
        ...
        <div className="modal-overlay" onClick={(e) => { if(e.target.className === 'modal-overlay') setIsNoticeModalOpen(false) }}>
          <div className="modal-content" style={{ maxWidth: 500 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32 }}>
              ...
              <button className="btn btn-secondary" style={{ padding: 8, borderRadius: 12, border: 'none' }} onClick={() => setIsNoticeModalOpen(false)}>
                <X size={24} />
              </button>
            </div>
          ...
          <div className="modal-content" style={{ maxWidth: 450, padding: '40px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                <div style={{ width: 64, height: 64, borderRadius: 32, background: selectedEmpInfo.status === 'ACTIVE' ? 'linear-gradient(135deg, var(--primary), #a855f7)' : 'linear-gradient(135deg, var(--text-muted), #333)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 28, fontWeight: 900, color: '#fff' }}>
              ...
              <button className="btn btn-secondary" style={{ padding: 8, borderRadius: 12, border: 'none' }} onClick={() => setSelectedEmpInfo(null)}>
                <X size={24} />
              </button>
            </div>
        ...
        <div className="modal-overlay" onClick={(e) => { if(e.target.className === 'modal-overlay') setIsUploadSlipModalOpen(false) }}>
          <div className="modal-content" style={{ maxWidth: 500 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32 }}>
              ...
              <button className="btn btn-secondary" style={{ padding: 8, borderRadius: 12, border: 'none' }} onClick={() => setIsUploadSlipModalOpen(false)}>
                <X size={24} />
              </button>
            </div>
      ...
      <div style={{ position: 'fixed', bottom: 30, right: 30, zIndex: 9999, display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
        {isCopilotOpen && (
          <div style={{ 
            width: 380, height: 500, background: 'var(--bg-card)', border: '1px solid var(--border)', 
            borderRadius: 24, boxShadow: '0 20px 40px rgba(0,0,0,0.2)', marginBottom: 16,
            display: 'flex', flexDirection: 'column', overflow: 'hidden'
          }}>
            <div style={{ padding: '16px 20px', background: 'linear-gradient(135deg, var(--primary), #a855f7)', color: '#fff', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <Bot size={22} />
                <span style={{ fontWeight: 800, fontSize: 16 }}>Pulse AI Copilot</span>
              </div>
              <button style={{ background: 'transparent', border: 'none', color: '#fff', cursor: 'pointer' }} onClick={() => setIsCopilotOpen(false)}>
                <X size={20} />
              </button>
            </div>
            ...
            <form onSubmit={handleCopilotSubmit} style={{ padding: 12, borderTop: '1px solid var(--border)', display: 'flex', gap: 8, background: 'var(--bg-card)' }}>
              <input 
                type="text" 
                placeholder="Ask AI to automate tasks..." 
                value={copilotInput}
                onChange={(e) => setCopilotInput(e.target.value)}
                style={{ flex: 1, padding: '12px 16px', borderRadius: 16, border: '1px solid var(--border)', background: 'var(--bg-input)', color: 'var(--text-main)', outline: 'none' }}
              />
              <button type="submit" disabled={!copilotInput.trim()} style={{ background: 'var(--primary)', color: '#fff', border: 'none', padding: '12px', borderRadius: 16, cursor: copilotInput.trim() ? 'pointer' : 'not-allowed', opacity: copilotInput.trim() ? 1 : 0.5, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Send size={18} />
              </button>
            </form>
          </div>
        ...
        {!isCopilotOpen && (
          <button 
            ...
            style={{ 
              width: 65, height: 65, borderRadius: 35, background: 'linear-gradient(135deg, var(--primary), #a855f7)', 
              color: '#fff', border: 'none', cursor: 'pointer', boxShadow: '0 10px 25px rgba(99, 102, 241, 0.4)',
